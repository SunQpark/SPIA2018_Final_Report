...
the model to generate varied images, so the marginal R
\[p(y|x = G(z))\]dz should have high entropy.
Combining these two requirements, the metric that we propose is: \[exp(E_xKL(p(y|x)||p(y)))\], where
we exponentiate results so the values are easier to compare."
While there are some of defeats ~\cite{note_on_inception}%TODO: rephrase
